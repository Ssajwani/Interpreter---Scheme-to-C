...
Value *parse(Value *tokens) {
    ...
    while (current->type != NULL_TYPE) { // while there are still tokens
        ...
        //     if (strcmp(current -> c.car -> s, '#t') == 0) { // checking to see whether it is true
        //         tree -> c.car -> type = BOOL_TYPE; // assign the type
        //         tree -> c.car -> s = current -> c.car -> s; // carry over the string (true)
        //     }
        // }
        current = cdr(current); 
    }
